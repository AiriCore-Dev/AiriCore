...
def dog_tag(
    ...
    if  Plugin_Config.SHOW_SPECIAL_TAG is True and os.path.exists(special_png_path):
        symbol = Picture.imread(special_png_path)
        x1 = 98
        ...
        symbol = Picture.imread(custom_png_path)
        symbol = Picture.resize(symbol, 0.818, 0.818)
        x2 = x1 + symbol.shape[1]
        y2 = y1 + symbol.shape[0]
        img = Picture.merge_img(img, symbol, y1, y2, x1, x2)
        del symbol
        ...
        background = Picture.imread(background_png_path)
        background = Picture.resize(background, 0.818, 0.818)
        ...
        border = Picture.imread(border_png_path)
        border = Picture.resize(border, 0.818, 0.818)
        ...
        symbol = Picture.imread(symbol_png_path)
        symbol = Picture.resize(symbol, 0.818, 0.818)
        x2 = x1 + symbol.shape[1]
        y2 = y1 + symbol.shape[0]
        img = Picture.merge_img(img, symbol, y1, y2, x1, x2)
