...
def visible_routes(personal: PersonalState, world: WorldState, date_context: CalendarContext) -> tuple[Route, ...]:
    ...
    for location_id in candidates:
        ...
        overlay = world.map_overlays.get(location_id)
        if unlocked:
            result.append(Route(location_id, record["terrain"], record["risk"] + (1 if overlay == "danger" else 0), (record["resources"],), True))
    return tuple(result)
...
def enter_location(personal: PersonalState, world: WorldState, date_context: CalendarContext, location_id: str) -> Encounter:
    ...
    return Encounter(location_id, encounter_id, ("探索", "谨慎前进", "返回希望号"), (100 + random.randrange(151),))
