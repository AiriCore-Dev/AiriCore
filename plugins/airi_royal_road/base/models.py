...
class CombatState:
    ...
    FIELDS: ClassVar[set[str]] = {"encounter_id", "enemy_id", "turn", "player_hp", "enemy_hp", "statuses", "pending_action", "resolved", "result", "action_receipts"}
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "action_receipts": {key: dict(value) for key, value in self.action_receipts.items()},
        }
    ...
    def from_dict(cls, data: dict[str, Any]) -> "CombatState":
        ...
        result = _dict(data["result"])
        if not _plain_result(result):
            raise TypeError("战斗结果必须是纯对象")
        action_receipts = _dict(data["action_receipts"])
        if any(not isinstance(value, dict) or not _plain_result(value) for value in action_receipts.values()):
            raise TypeError("战斗回执必须是纯对象")
        return cls(_str(data["encounter_id"]), _str(data["enemy_id"]), _int(data["turn"]), _int(data["player_hp"]), _int(data["enemy_hp"]), _string_list(data["statuses"]), pending, _bool(data["resolved"]), result, action_receipts)
...
class WeeklyRun:
    ...
    FIELDS: ClassVar[set[str]] = {"week_index", "location_id", "fire", "injuries", "recalls_used", "action_dates", "temporary_items", "blessings", "encounter"}
    ...
    def from_dict(cls, data: dict[str, Any]) -> "WeeklyRun":
        data = _keys(data, cls.FIELDS)
        encounter = _dict(data["encounter"])
        if not _plain_result(encounter):
            raise TypeError("遭遇记录必须是纯对象")
        return cls(_int(data["week_index"]), _str(data["location_id"]), _int(data["fire"]), _string_list(data["injuries"]), _int(data["recalls_used"]), _string_list(data["action_dates"]), _string_list(data["temporary_items"]), _string_list(data["blessings"]), encounter)
...
class VoteBox:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "VoteBox":
        ...
        result = _dict(data["result"])
        if not _plain_result(result):
            raise TypeError("投票结果必须是纯对象")
        return cls(_int(data["week_index"]), _string_list(data["eligible_users"]), _string_dict(data["choices"]), _bool(data["opened"]), _bool(data["closed"]), _str(data["hope_priority"]), result)
...
class PersonalState:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "PersonalState":
        ...
        if schema_version != SCHEMA_VERSION:
            raise ValueError("个人存档版本不受支持")
        return cls(_str(data["user_id"]), schema_version, job, parsed_bonds, InventoryState.from_dict(data["inventory"]), WeeklyRun.from_dict(data["weekly"]), combat, _int(data["current_emblems"]), _int(data["total_emblems"]), _string_list(data["event_ids"]), data["last_render_key"] if data["last_render_key"] is None else _str(data["last_render_key"]), _str(data["settlement_status"]), pending_choice, _string_list(data["partner_action_dates"]), settlement_id, _int(data["settlement_amount"]))
...
class WorldState:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "WorldState":
        ...
        schema_version = _int(data["schema_version"])
        if schema_version != SCHEMA_VERSION:
            raise ValueError("世界存档版本不受支持")
        return cls(schema_version, _string_dict(data["map_overlays"]), {key: VoteBox.from_dict(value) for key, value in votes.items()}, _string_list(data["completed_events"]), results, _string_list(data["event_ids"]), _bool(data["frozen"]))
