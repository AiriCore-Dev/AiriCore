...
def format_msg(msg_list: List[Union[List[str], str]], is_plain_text: bool = False):
    ...
    if is_plain_text:
        msgres = MessageSegment.text("")
        for i in flatten_msg_list:
            if not is_image(i):
                msgres += MessageSegment.text(i)
            else:
                msgres += MessageSegment.image(i)
        return msgres
...
async def send_msg(msg_list: List[Union[List[str], str, bool]], event) -> None:
    if msg_list is False:
        return
    if msg_list is None:
        logger.warning("此次解析的内容为空，接口可能被修改，需要更新！")
    ...
    try:
        ...
        # 2. 构造两条消息内容
        first_msg = "B站链接解析"
        second_msg = format_msg(msg_list, is_plain_text=True)  # 复用你原有的格式化函数

        # 3. 构造合并转发节点（可自定义 QQ 和昵称）
        forward_nodes = []
        ...
        if isinstance(event, GroupMessageEvent):
            ...
            logger.warning("无法识别的消息事件类型，未发送合并转发")
