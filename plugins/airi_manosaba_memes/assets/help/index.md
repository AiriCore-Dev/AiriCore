...
查看帮助、立绘选择表和背景免费。安安说、魔裁鸭梨、立绘成图及魔裁对话每次制作 **10 积分**；回复已有立绘继续编辑免费。发送失败会退还本次制作积分。
...
先收到免费的官方预设选择表，回复选择表发送 `P1` 生成第一套立绘。
...
免费浏览背景并取得背景短码。背景短码以 `@BG` 开头，与立绘短码 `#` 区分。

```text
魔裁背景 场景 1
```

每页 **5×5，最多 25 张**。回复选择表发送 `B1` 查看对应大图和短码，发送 `上一页` 或 `下一页` 翻页，编号跨页连续。
...
- 原插件作者：[zhaomaoniu](https://github.com/zhaomaoniu/)。
