...
def render_evidence(request, uploaded_image: bytes | None = None) -> bytes:
    ...
    lines = wrap_description(request.description)
    if uploaded_image is None:
        entry = ItemCatalog().get(request.item)
        source = get_source(entry.path)
        item_image = _image(entry.path)
    else:
        source = uploaded_image
        item_image = decode_upload(source)
    descriptor = json.dumps({'kind': 'evidence', 'layout': 1, 'source': hashlib.sha256(source).hexdigest(),
        'name': name, 'description': request.description, 'font': _signature(FONT),
