...
def _record_has_legacy_images(record: dict) -> bool:
    for value in record.get("images", []):
        if isinstance(value, str) and value.startswith("base64://"):
            return True
    for segment in record.get("segments", []):
...
def has_legacy_images(data: dict) -> bool:
    for section in ("bottles", "pending_bottles"):
        records = data.get(section, {})
        if not isinstance(records, dict):
            continue
        if any(
            isinstance(record, dict) and _record_has_legacy_images(record)
...
def _migrate_record_sync(record: dict) -> bool:
    original_images = list(record.get("images", []))
    migrated_images = []
    changed = False
    for value in original_images:
        if not isinstance(value, str) or not value.startswith("base64://"):
            migrated_images.append(value)
            continue
        try:
            migrated = _write_image_sync(value)
        except Exception as e:
            logger.warning(f"心愿瓶旧图片迁移失败，保留 base64: {e}")
            migrated = value
        else:
            changed = True
        migrated_images.append(migrated)

    original_segments = record.get("segments")
    migrated_segments = []
    image_index = 0
    if isinstance(original_segments, list):
        for segment in original_segments:
            ...
            result = dict(segment)
            if result.get("type") == "image":
                if image_index < len(migrated_images):
                    replacement = migrated_images[image_index]
                    image_index += 1
                else:
                    ...
                    if isinstance(value, str) and value.startswith("base64://"):
                        try:
                            replacement = _write_image_sync(value)
                        except Exception as e:
                            logger.warning(f"心愿瓶旧图文段迁移失败，保留 base64: {e}")
                        else:
                            migrated_images.append(replacement)
                            changed = True
                if result.get("data") != replacement:
                    result["data"] = replacement
                    changed = True
            migrated_segments.append(result)

    if migrated_images != original_images:
        record["images"] = migrated_images
    if isinstance(original_segments, list) and migrated_segments != original_segments:
        record["segments"] = migrated_segments
    return changed


def _migrate_records_sync(data: dict) -> bool:
    changed = False
    for section in ("bottles", "pending_bottles"):
        records = data.get(section, {})
        if not isinstance(records, dict):
            continue
        for record in records.values():
            if isinstance(record, dict) and _migrate_record_sync(record):
                changed = True
    return changed


async def migrate_records(data: dict) -> bool:
    return await asyncio.to_thread(_migrate_records_sync, data)
